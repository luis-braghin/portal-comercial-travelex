...
def mostrar_formularios(titulo, lista):
    """Função específica para mostrar formulários com botões de preenchimento e acompanhamento"""
    if titulo:
        st.markdown(f"<div class='section-title'>{titulo}</div>", unsafe_allow_html=True)
    
    for nome, link_form, link_list in lista:
        # Criar o botão de acompanhamento apenas se existir o link
        btn_acompanhar = ""
        if link_list:
            btn_acompanhar = f"""
                <a href="{link_list}" target="_blank" class="btn-form btn-tracking">
                    📊 Acompanhar Status
                </a>
            """
            ...
            <div class="form-card">
                <div class="form-card-content">
                    ...
                    <div class="form-buttons">
                        <a href="{link_form}" target="_blank" class="btn-form">
...
def buscar_conteudos(termo):
    ...
    for secao, itens in conteudos.items():
        if secao == "📄 Formulários":
            # Formulários têm 3 elementos (nome, link_form, link_list)
            filtrados = [(nome, link, link_list) for nome, link, link_list in itens if termo in nome.lower()]
        else:
            # Outras seções têm 2 elementos (nome, link)
...
if selected == "🏠 Início":
    ...
    if termo:
        ...
        for secao in conteudos:
            if secao == "📁 KYC e Documentos de Abertura":
                ...
                mostrar_formularios(secao, conteudos[secao])
            else:
    ...
    if secao_nome:
        # Usar função específica para formulários
        if secao_nome == "📄 Formulários":
            mostrar_formularios(secao_nome, conteudos[secao_nome])
        else:
